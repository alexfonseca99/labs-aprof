...
class CNN(nn.Module):
    ...
    def __init__(self, p_dropout):
        ...
        self.convblock1 = nn.Sequential(
            nn.Conv2d(1, 16, (3, 3), padding=1),  #escolher tipo de padding
            #H=W=28
            nn.ReLU(),
...
def plot_kernels(convolutional_layer_1, convolutional_layer_2, name_1='', name_2=''):
    ...
    plt.savefig('%s.eps' % (name_1), format='eps', bbox_inches='tight')
    ...
    plt.savefig('%s.eps' % (name_2), format='eps',bbox_inches='tight')
...
def main():
    ...
    filename = "results/" + config + ".txt"
    with open(filename, 'w') as f:
        f.write("train_mean_losses = [")
        for el in train_mean_losses:
            f.write(str(el) + ", ")
        f.write("]\n")
        f.write("valid_accs = [")
        for el in valid_accs:
            f.write(str(el) + ", ")
        f.write("]\n")
        f.write('Final Test acc: %.4f\n' % (test_acc))
        f.write("Time: %d" % (end_time - start_time))
